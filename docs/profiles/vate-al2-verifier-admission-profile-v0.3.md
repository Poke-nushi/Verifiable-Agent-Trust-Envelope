...
This document is a `v0.3` profile proposal.
...
VATE v0.3 stays adjacent to A2A, MCP, OAuth, VC, DID, OID4VP, Web Bot Auth, AP2, x402, ACP, and payment-token systems.
...
For this `v0.3` AL2 profile, `evidence_refs` is required and must contain at
...
That remains a useful mental model, but v0.3 treats verification as dependency-aware stages rather than a rigid linear order.
...
The v0.3 receipt split is described in:

- [docs/receipt-model-v0.3.md](../receipt-model-v0.3.md)
